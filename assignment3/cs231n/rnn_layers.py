...
def rnn_step_backward(dnext_h, cache):
    ...
    dtemp = (1 - np.square(next_h)) * dnext_h
...
def rnn_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
      if i == 0:
        cache_step = x[:, i, :], Wx, h0, Wh, b, h[:, i, :]
      else:
        cache_step = x[:, i, :], Wx, h[:, i - 1, :], Wh, b, h[:, i, :]
...
def word_embedding_backward(dout, cache):
    ...
    dW = np.zeros(W.shape)
    x = x.reshape((N * T))
    dout = (np.transpose(dout, (2, 0, 1))).reshape((D, -1))
    dout = np.transpose(dout, (1, 0))
    np.add.at(dW[:], x, dout[:])
    pass
