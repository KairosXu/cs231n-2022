...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        loss1 = -torch.log((torch.exp(sim(z_k, z_k_N) / tau)) / (torch.sum(torch.Tensor([torch.exp(sim(z_k,out[i])/tau)for i in range(2*N)]))-torch.exp(sim(z_k,z_k)/tau)) )
        loss2 = -torch.log((torch.exp(sim(z_k_N, z_k) / tau)) / (torch.sum(torch.Tensor([torch.exp(sim(z_k_N,out[i])/tau)for i in range(2*N)]))-torch.exp(sim(z_k_N,z_k_N)/tau)) )
